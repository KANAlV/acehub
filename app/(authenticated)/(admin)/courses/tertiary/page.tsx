...
import {
    ...
    getAllProgramsData,
    updateProgram,
    deleteProgram, fetchProgramCountCollege, fetchProgramsCollege
...
const StudentInputs = ({ studentsVal, handleStudentChange }: { 
...
export default function CoursesManager() {
    ...
    const [warningType, setWarningType] = useState(""); // Track What warning will show
     ...
     const academicLevelVal = "College"; // College only
     
     // Student counts per year level (College only: years 1, 2, 3, 4)
     const [studentsVal, setStudentsVal] = useState<Record<string, string>>({
         "1": "", "2": "", "3": "", "4": ""
     });

     // Track original values for comparison
     const [selectedProgramOld, setSelectedProgramOld] = useState("");
     const [programNameValOld, setProgramNameValOld] = useState("");
     const [studentsValOld, setStudentsValOld] = useState<Record<string, string>>({
         "1": "", "2": "", "3": "", "4": ""
     });
    ...
    const totalPageCount = Math.max(
        1,
        Math.ceil(safeRowCount / itemsPerPage)
    );

    const startItem = safeRowCount > 0
        ? ((currentPage - 1) * itemsPerPage) + 1
        : 0;

    const endItem = Math.min(currentPage * itemsPerPage, safeRowCount);
    ...
    /** UI Functions **/
     function editModalValue(id: string) {
         ...
         // Ensure students is handled as an object (College only: years 1, 2, 3, 4)
         ...
         const newStudents: Record<string, string> = {
             "1": dbStudents["1"]?.toString() || "",
             "2": dbStudents["2"]?.toString() || "",
             "3": dbStudents["3"]?.toString() || "",
             "4": dbStudents["4"]?.toString() || ""
         };
         
         setStudentsVal(newStudents);
         
         // Store original values for comparison
         setSelectedProgramOld(program.program_code);
         setProgramNameValOld(program.program_name);
         setStudentsValOld(newStudents);
    ...
    function showWarning(color: string) {
     ...
     function discardEntry() {
         ...
         setProgramNameValOld("");
         setStudentsValOld({ "1": "", "2": "", "3": "", "4": "" });
    ...
    function filterProgramName(e:string) {
     ...
     function handleStudentChange(year: string, val: string) {
         setStudentsVal(prev => ({
             ...prev,
             [year]: clampNumericValue(val, 700, prev[year])
         }));
         setActiveChanges(true);
     }

    function handleLevelChange(level: string) {
        // Level is now fixed to College, no need to change it
        setStudentsVal({ "1": "", "2": "", "3": "", "4": "" });
        setActiveChanges(true);
    }
    ...
    const loadProgramData = async () => {
        const data = await fetchProgramsCollege(search, currentPage);
        setPrograms(data);
        setTableLoading(false);
    }

    const loadProgramCount = async () => {
        const rowCount = await fetchProgramCountCollege(search);
    ...
    useEffect(() => {
     ...
     useEffect(() => {
         const hasChanges =
             selectedProgram !== selectedProgramOld ||
             programNameVal !== programNameValOld ||
             studentsVal["1"] !== studentsValOld["1"] ||
             studentsVal["2"] !== studentsValOld["2"] ||
             studentsVal["3"] !== studentsValOld["3"] ||
             studentsVal["4"] !== studentsValOld["4"];

         setActiveChanges(hasChanges);
     }, [selectedProgram, programNameVal, studentsVal, selectedProgramOld, programNameValOld, studentsValOld]);
            ...
            <Modal size={"md"} show={addModal} initialFocus={AddModalProgramNameInput} onClose={() => setAddModal(false)}>
                ...
                <ModalBody>
                    ...
                    <div className="mt-4">
                        ...
                        <p className="text-gray-700 dark:text-gray-300">College</p>
                    </div>
                    <StudentInputs studentsVal={studentsVal} handleStudentChange={handleStudentChange} />
                </ModalBody>

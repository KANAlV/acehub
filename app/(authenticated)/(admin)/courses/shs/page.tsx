...
import {
    ...
    getAllProgramsData,
    updateProgram,
    deleteProgram, fetchProgramCountShs
...
export default function CoursesManager() {
    ...
    const [warningType, setWarningType] = useState(""); // Track What warning will show
     ...
     const academicLevelVal = "SHS"; // SHS only

     // Student counts per year level (SHS only: years 11 and 12)
    ...
    function editModalValue(id: string) {
         ...
         // Ensure students is handled as an object (SHS only: years 11 and 12)
         ...
         const newStudents: Record<string, string> = {
             "11": dbStudents["11"]?.toString() || "",
             "12": dbStudents["12"]?.toString() || ""
         };
    ...
    function handleLevelChange(level: string) {
        // Level is now fixed to SHS, no need to change it
        setStudentsVal({ "11": "", "12": "" });
        setActiveChanges(true);
    }
    ...
    async function handleProgramImport(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            worksheet?.eachRow((row, rowNumber) => {
                ...
                // Filter for SHS only
                if (level !== "SHS") {
                    console.warn(`[IMPORT_WARNING]: Skipping row ${rowNumber} - Level "${level}" is not SHS`);
                    return;
                }
    ...
    const loadProgramData = async () => {
        const data = await fetchProgramsShs(search, currentPage);
        setPrograms(data);
        setTableLoading(false);
    }

    const loadProgramCount = async () => {
        const rowCount = await fetchProgramCountShs(search);
    ...
    useEffect(() => {
     ...
     useEffect(() => {
         const hasChanges =
             ...
             studentsVal["11"] !== studentsValOld["11"] ||
             studentsVal["12"] !== studentsValOld["12"];
            ...
            <Modal size={"md"} show={addModal} initialFocus={AddModalProgramNameInput} onClose={() => setAddModal(false)}>
                ...
                <ModalBody>
                    ...
                    <div className="mt-4">
                        ...
                        <p className="text-gray-700 dark:text-gray-300">SHS</p>
                    </div>
